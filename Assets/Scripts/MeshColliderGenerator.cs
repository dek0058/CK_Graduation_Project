...
public class MeshColliderGenerator : MonoBehaviour {
    ...
    public void mc_combine ( ) {
        if(mc_targets.Count == 0) {
            return;
        }
        CombineInstance[] combine = new CombineInstance[mc_targets.Count + 1];
        for(int i = 0; i < mc_targets.Count; ++i ) {
            combine[i].mesh = mc_targets[i].sharedMesh;
            combine[i].transform = mc_targets[i].transform.localToWorldMatrix;
        }
        combine[combine.Length - 1].mesh = mesh;
        combine[combine.Length - 1].transform = transform.localToWorldMatrix;
    ...
    public void m_combine ( ) {
        ...
        CombineInstance[] combine = new CombineInstance[m_targets.Count + 1];
        for ( int i = 0; i < m_targets.Count; ++i ) {
            combine[i].mesh = m_targets[i].sharedMesh;
            combine[i].transform = m_targets[i].transform.localToWorldMatrix;
        }
        combine[combine.Length - 1].mesh = mesh;
        combine[combine.Length - 1].transform = transform.localToWorldMatrix;
        Mesh new_mesh = new Mesh ( );
...
public class MeshColliderGeneratorEditor : Editor {
    public override void OnInspectorGUI ( ) {
        ...
        MeshColliderGenerator t = target as MeshColliderGenerator;
        if(GUILayout.Button( "Simplify" ) ) {
            t.simplify ( );
        }

        if ( GUILayout.Button ( "MC_Combine" ) ) {
            t.mc_combine ( );
        }

        if ( GUILayout.Button ( "M_Combine" ) ) {
            t.m_combine ( );
        }
